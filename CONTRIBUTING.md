...
- **`data/private/`** — real participant data. This study collected keystroke timings from
  49 people under consent; their data does not leave the machine it was collected on.
...
- **Report sample counts and confidence intervals on every number.** A table row without an *n*
  is not a result.
- **Include baselines you lose to.** Killourhy & Maxion (2009) report EER **0.0962** with scaled
  Manhattan distance on the same 51 CMU subjects; our best is currently **0.1367**. That gap is
...
| Location | Defect |
|---|---|
| `edge/reference/capture.js.BUGGY` | Keys events by `selectionStart`, which sits before the inserted character on keydown and after it on keyup — so every keydown was matched to the **previous** character's keyup. 85% of recorded human dwell times are negative (median −285.5 ms). Key events by `e.code` plus a monotonic press counter instead. |
